...
public class BlobStorageManager : MonoBehaviour
{
    ...
    public async Task UploadOrUpdateBlockBlobAsync(string pathFileDevice, string pathContainer, string nameFile, Action actionBefore = null, Action actionAfter = null)
    {
        CloudBlockBlob blockBlob = GetBlockBlobReference(pathContainer, nameFile);
        actionBefore?.Invoke();
    ...
    public async Task UpdateFileTxtBlockBlobAsync(string nameContainer, string pathFileDevice, string pathFileInContainer, string nameFile, string contentText, Action actionBefore = null, Action actionAfter = null)
    {
        actionBefore?.Invoke();
        File.WriteAllText(Path.Combine(pathFileDevice, nameFile), contentText);
        var container = GetContainerReference(nameContainer); // Error first time
        var blockBlob = container.GetBlockBlobReference(Path.Combine(pathFileInContainer + nameFile));
        await blockBlob.UploadFromFileAsync(Path.Combine(pathFileDevice, nameFile));
        actionAfter?.Invoke();
    }
